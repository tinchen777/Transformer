...
d_qk = 64  # dimension of K(=Q), V（Q和K的维度需要相同，这里为了方便让K=V）
d_v = 64
...
model = Transformer(
    ...
    enc_config=ModuleConfig(pad_idx=src_vocab['P']),
    dec_config=ModuleConfig(pad_idx=tgt_vocab['P'])
).to(device)
...
for epoch in range(epochs):
    for enc_inputs, dec_inputs, dec_outputs in loader:
        ...
        # dec_outputs.view(-1):[bsz * tgt_len * tgt_vocab_size]
        loss = criterion(
...
#                               -1)
#         dec_outputs = model.decoder(dec_input, enc_outputs, src_mask=model.encoder.get_pad_mask(enc_input))
...
def test(model: Transformer, enc_input, start_symbol):
    # Starting Reference: http://nlp.seas.harvard.edu/2018/04/03/attention.html#greedy-decoding
    model.eval()
    with torch.no_grad():
        enc_outputs = model.encoder(enc_input)
        ...
        for i in range(0, tgt_len):
            ...
            dec_outputs = model.decoder(dec_input, enc_outputs, src_mask=model.encoder.make_pad_mask(enc_input))
...
print(enc_inputs)  # Encoder输入的索引

predict_dec_input = test(model, enc_inputs[0].view(1, -1).to(device), start_symbol=tgt_vocab["S"])

print(predict_dec_input)  # Decoder输入的索引（预测阶段）

print("other:")
predict = model(enc_inputs[0].view(1, -1).to(device), predict_dec_input)
print(predict)  # Decoder输出的logits（预测阶段)
